#pragma once
class GcPropertyGridProperty  : public CMFCPropertyGridProperty
{
protected:
	...
	CString mButtonName;
	bool mFloatSpin;
	GnSignal1<GcPropertyGridProperty*> mUpdateEvent;

public:
	GcPropertyGridProperty(const CString& strGroupName, DWORD_PTR dwData = 0, BOOL bIsValueList = FALSE);

	GcPropertyGridProperty(const CString& strName, const COleVariant& varValue, LPCTSTR lpszDescr = NULL, DWORD_PTR dwData = 0,
		LPCTSTR lpszEditMask = NULL, LPCTSTR lpszEditTemplate = NULL, LPCTSTR lpszValidChars = NULL);
	virtual ~GcPropertyGridProperty(void);

	// 기본값을 셋팅한다.
	void SetDefaultValue();
	void SetEnableGroupButton(bool val, LPCTSTR pcName = NULL);
	void SubscribeToUpdateEvent( GnBaseSlot1<GcPropertyGridProperty*>* pSlot );
