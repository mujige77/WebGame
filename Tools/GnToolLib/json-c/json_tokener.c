  ...
  redo_char:
    ...
    case json_tokener_state_null:
      ...
      if(strncasecmp(json_null_str, tok->pb->buf,
		     json_min((size_t)tok->st_pos+1, strlen(json_null_str))) == 0) {
    ...
    case json_tokener_state_boolean:
      ...
      if(strncasecmp(json_true_str, tok->pb->buf,
		     json_min((size_t)tok->st_pos+1, strlen(json_true_str))) == 0) {
	if(tok->st_pos == strlen(json_true_str)) {
	  ...
	  goto redo_char;
			    ...
			    json_min((size_t)tok->st_pos+1, strlen(json_false_str))) == 0) {
