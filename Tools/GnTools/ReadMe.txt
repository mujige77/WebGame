================================================================================
    MFC 라이브러리 : GnTools 프로젝트 개요
================================================================================

응용 프로그램 마법사에서 이 GnTools 응용 프로그램을 
만들었습니다. 이 응용 프로그램은 MFC의 기본 사용법을 보여 줄 뿐만 아니라 응용 
프로그램작성을 위한 기본 구조를 제공합니다.

이 파일에는 GnTools 응용 프로그램을 구성하는 각 파일에 대한
요약 설명이 포함되어 있습니다.
...
GnTools.h
    ...
    헤더(Resource.h 포함)가 들어 있고 CGnToolsApp 응용 프로그램
    클래스를 선언합니다.

GnTools.cpp
    응용 프로그램 클래스 CGnToolsApp이(가) 들어 있는 기본 응용 프로그램
    소스 파일입니다.
...
res\GnTools.ico
    ...
    주 리소스 파일인 GnTools.rc에 의해 포함됩니다.
...
GnToolsDoc.h, GnToolsDoc.cpp - 문서
    이 파일에는 CGnToolsDoc 클래스가 들어 있습니다. 이 파일을 편집하여
    특수한 문서 데이터를 추가하고 CGnToolsDoc::Serialize를 통해 파일
    저장 및 로드를 구현합니다.
    이 문서에는 다음 문자열이 포함됩니다.
        파일 확장명:      gns
        파일 형식 ID:        GnTools.Document
        주 프레임 캡션:  GnTools
        문서 형식 이름:       GnTools
        필터 이름:         GnTools Files (*.gns)
        파일의 새 약식 이름: GnTools
        파일 형식의 긴 이름: GnTools.Document

GnToolsView.h, GnToolsView.cpp - 문서 뷰
    이 파일에는 CGnToolsView 클래스가 들어 있습니다.
    CGnToolsView 개체는 CGnToolsDoc 개체를 보는 데 사용됩니다.

res\GnToolsDoc.ico
    CGnToolsDoc 클래스의 MDI 자식 창에 대한 아이콘으로 사용되는
    아이콘 파일입니다. 이 아이콘은 주 리소스 파일인 GnTools.rc에
    의해 포함됩니다.
...
GnToolLibPCH.h, GnToolLibPCH.cpp
    이 파일은 미리 컴파일된 헤더 파일(PCH)인 GnTools.pch와
    미리 컴파일된 형식 파일인 GnToolLibPCH.obj를 빌드하는 데 사용됩니다.
