...
GtBoundedNumberPairProp::GtBoundedNumberPairProp(const CString& strGroupName, int nMinValue1, int nMaxValue1
	, int nMinValue2, int nMaxValue2, DWORD_PTR dwData, BOOL bIsValueList)  : GtPropertyGridProperty(strGroupName
	, dwData, bIsValueList)
{
...
GtBoundedNumberPairProp::~GtBoundedNumberPairProp(void)
{
}

BOOL GtBoundedNumberPairProp::OnUpdateValue()
{
	ASSERT_VALID(this);
	...
	if (bIsChanged)
	{
		...
		return GtPropertyGridProperty::OnUpdateValue();
	}
...
GtBoundedNumberSubProp::GtBoundedNumberSubProp(const CString& strName, const COleVariant& varValue, int nMinValue
	, int nMaxValue, LPCTSTR lpszDescr) : GtPropertyGridProperty(strName, varValue, lpszDescr)
{
...
BOOL GtBoundedNumberSubProp::OnUpdateValue()
{
	...
	if (bIsChanged)
	{
		...
		bRet = GtPropertyGridProperty::OnUpdateValue();
