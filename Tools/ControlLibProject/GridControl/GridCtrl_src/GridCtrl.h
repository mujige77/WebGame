...
#define GVN_LEAVEDRAG           LVN_BEGINRDRAG        // LVN_FIRST-9
...
#define SaveClass( classname ) \
	CGridCtrl::FunctionRegSave( _T(#classname), classname::CreateCtrl )	
...
public:
	BOOL		SetDragSelection(BOOL val){ m_bDragSelection = val; }
	BOOL		GetDragSelection(){ return m_bDragSelection; }
	typedef CGridCtrl* (*CreateFunction)(void);
	static std::map< ctrstring, CreateFunction >	m_CtrlReg;	
	static void	FunctionRegSave( ctrstring strClassName, CreateFunction Function )
	{		
		m_CtrlReg.insert( make_pair( strClassName, Function ) );
	}
	static CreateFunction FunctionRegGet( ctrstring strClassName )
	{
		std::map< ctrstring, CreateFunction >::iterator iterFunction;
		iterFunction = m_CtrlReg.find( strClassName );
		if ( iterFunction != m_CtrlReg.end() )
