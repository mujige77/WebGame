...
CPropTree::CPropTree( bool bFontBold  ) :
	m_bShowInfo(TRUE),
	m_nInfoHeight(50),
	m_pVisbleList(NULL),
	m_Origin(100,0),
...
int CPropTree::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	m_nPropUID = (UINT)::GetMenu(m_hWnd);
	DWORD dwStyle;
	CRect rc;
	...
	dwStyle = WS_VISIBLE|WS_CHILD|WS_VSCROLL|WS_VSCROLL ;
	...
	//{		
		dwStyle &= ~WS_VSCROLL;		
		m_pInfo.Create( _T(""), dwStyle, rc, this );
	//}
	//else
	//	m_pInfo.m_pInfo->ShowWindow( SW_SHOW );
		
	m_pInfo.SetPropOwner(this);


	return 0;
}
void CPropTree::SetDailog( CDialog* pDlg )
{
	if( m_pInfo.m_pInfo )
		m_pInfo.m_pInfo->ShowWindow( SW_HIDE );
	if ( pDlg )
	{
		pDlg->SetParent( this );
...
void CPropTree::ResizeChildWindows(int cx, int cy)
{
	if (m_bShowInfo)
	{
		...
		if ( m_pInfo.m_pInfo )
		{
			if ( IsWindow( m_pInfo.m_pInfo->m_hWnd ) )			
				m_pInfo.m_pInfo->MoveWindow(0, cy - m_nInfoHeight, cx, m_nInfoHeight);
			
		}
		else if ( IsWindow(m_pInfo.m_hWnd) )
			m_pInfo.MoveWindow(0, cy - m_nInfoHeight, cx, m_nInfoHeight);
	}
	else
	{
		if (IsWindow(m_List.m_hWnd))
...
LONG CPropTree::HitTest(const POINT& pt)
{
	...
	{
		// 분리선을 움직일수 있는지 없는지를 판단한다. pItem->IsRootLevel() || pItem->m_bNoRoot
		if ( pItem->GetSeparator() &&
			( !pItem->IsRootLevel() || pItem->GetRootState() ) &&
			pt.x >= m_Origin.x - PROPTREEITEM_COLRNG &&
			pt.x <= m_Origin.x + PROPTREEITEM_COLRNG )			 
			return HTCOLUMN;
...
LRESULT CPropTree::SendNotify(UINT nNotifyCode, CPropTreeItem* pItem)
{
	...
	{		
		...
		lpnm->idFrom = m_nPropUID;
	
		return GetParent()->SendMessage(WM_NOTIFY, (WPARAM)m_nPropUID, (LPARAM)lpnm);
	}
