...
#if _MSC_VER >= 1400
#ifdef START_THREAD
// 쓰레드는 _tcsncpy_s_l자 붙은걸 사용하는게 좋을거 같다.!!
#define _ncsncpy _tcsncpy_s	// 8.0에서는 이걸, 쓰레드에서는 _tcsncpy_s_l 사용해야한다.
#define _ncscpy	 _tcscpy_s	// 8.0에서는 이걸, 쓰레드에서는 _tcscpy_s를 사용해야한다.
#else
#define _ncsncpy	_tcsncpy	// 싱글용
#define _ncscpy		_tcscpy		// 싱글용
#define	_ncscmp		_tcscmp		// 싱글용
#define _ncsclen	_tcsclen	// 싱글용
#define _ncscat		_tcscat		// 싱글용
#define gc(s)	_T(s)
#endif	//START_THREA	
#else
#define _ncsncpy _tcsncpy
#endif
class CPropTree;
...
public:
	// TreeItem states
	BOOL IsExpanded();				// 현재 선택되어진 리스트 컨트롤이 확장되어 있는지 숨겨져 있는지 확인한다.
	BOOL IsSelected();
	BOOL IsChecked();
	BOOL IsReadOnly();
	BOOL IsActivated();
	...
	void Expand(BOOL bExpand = TRUE);	// 현재 선택되어진 리스트 컨트롤을 확장및 숨김을 셋팅한다.
	...
	// 자신의 오브젝트를 생성한다.
	virtual CPropTreeItem* CreateItem();
	//
...
public:
	//
	BOOL				m_bRootState;	// 최상위 레벨의 아이템을 일반 차일드와 똑같이 쓰겠다는 뜻!!
	BOOL				m_bSeparator;	// 분할선을 사용할지 안할지를 선택한다.
	UINT				m_nSiblingID;	// 같은 레벨의 노드에서의 자신 인덱스
	UINT				GetSiblingID();
	void				SetSiblingID(UINT nID);
	BOOL				GetRootState();
	BOOL				GetSeparator();
