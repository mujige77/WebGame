...
/////////////////////////////////////////////////////////////////////////////
// CPropTreeList
COLORREF CPropTreeList::m_ExpandColumnColor = 0x00f95fc6;
...
void CPropTreeList::OnPaint() 
{
	...
	memdc.SelectObject(CreateSolidBrush(m_ExpandColumnColor));
...
BOOL CPropTreeList::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
	if (nHitTest==HTCLIENT)
	{
		...
		{
			case HTCOLUMN:
				SetCursor( AfxGetApp()->LoadCursor( MAKEINTRESOURCE(IDC_SPLITTER) ));
				return TRUE;
...
void CPropTreeList::OnLButtonDown(UINT, CPoint point) 
{
	...
	LONG nHit = m_pProp->HitTest(point);		// 클릭 되었을때 해야될 일을 정한다.
	...
	switch (nHit)
	{
		...
		case HTEXPAND:		// 확장및 숨김에 관련된 일을 한다.
			if ((pItem = m_pProp->FindItem(point))!=NULL)
			{
		...
		default:
			if ((pItem = m_pProp->FindItem(point))!=NULL)
			{
				...
				if ( ( nHit==HTATTRIBUTE && !pItem->IsRootLevel() ) 
					|| (nHit==HTATTRIBUTE && pItem->GetRootState() ) )
				{
...
void CPropTreeList::OnLButtonDblClk(UINT, CPoint point)
{
	...
	if ((pItem = m_pProp->FindItem(point))!=NULL)
	{
		LONG hit = m_pProp->HitTest(point);
		if( HTATTRIBUTE == hit )
			m_pProp->SendNotify( PTN_ITEMBUTTONDCLICK, pItem );
		...
		if( pItem->GetChild() )
		{
			...
			case HTATTRIBUTE:				
				if (!pItem->IsRootLevel() || pItem->GetRootState() )
				{					
				}
				break;
...
void CPropTreeList::OnKeyDown(UINT nChar, UINT, UINT) 
{
	...
	{
		case VK_RETURN:
			if ((pItem = m_pProp->GetFocusedItem())!=NULL && ( !pItem->IsRootLevel() || pItem->GetRootState() ) && !pItem->IsReadOnly())
			{
				pItem->Activate();
			}
			break;
...
void CPropTreeList::OnRButtonDown(UINT nFlags, CPoint point)
{
