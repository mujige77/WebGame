...
#define PTN_ITEMBUTTONDCLICK		(PTN_FIRST-13)

class CPropTree : public CWnd
{
// Construction
public:
	CPropTree( bool bFontBold = TRUE );
	virtual ~CPropTree();
...
public:
	...
	virtual LRESULT SendNotify(UINT nNotifyCode, CPropTreeItem* pItem = NULL);
	
	// 인포의 높이를 조정한다.
	void	SetInfoHeight( int Size );
	// 다이얼로그를 지정한다.
	void SetDailog( CDialog* pDlg );
...
protected:
	...
	void InitGlobalResources( bool bFontBold );
