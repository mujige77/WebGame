...
{
	for (; w>0; w-=2, x+=2)
		SetPixel(hdc, x, y, CPropTreeList::GetExpandColumnColor());
}
...
BOOL CPropTreeItem::GetSeparator()
{
	return m_bSeparator;
}
BOOL CPropTreeItem::GetRootState()
{
	return m_bRootState;
}
UINT CPropTreeItem::GetSiblingID()
{
	return m_nSiblingID;
}
void CPropTreeItem::SetSiblingID(UINT nID)
{
	m_nSiblingID = nID;
}
LONG CPropTreeItem::DrawItem(CDC* pDC, const RECT& rc, LONG x, LONG y)
{
	...
	if ( ( IsRootLevel() && !GetRootState() ) || !m_bSeparator )
	...
	// 최상위 레벨의 리스트 박스의 색상을 처리 해준다. 루트일때는 색상을 무저건 바꿔 주고 싶었다. ㅋ;;
	if (IsRootLevel() && !GetRootState() )
	{
		HGDIOBJ hOld = pDC->SelectObject(CreateSolidBrush(CPropTreeList::GetExpandColumnColor()));
		pDC->PatBlt(rc.left, drc.top, rc.right - rc.left + 1, drc.Height(), PATCOPY);
		pDC->SelectObject(hOld);
	}

	// calc/draw expand box position
	if (GetChild())		// 하위 노드가 있다면 X박스표시를 한다.
	{
	...
	// draw separators
	if ( m_bSeparator && ( !IsRootLevel() || GetRootState()  ))
	{
		// column sep
...
CPropTreeItem* CPropTreeItem::CreateItem()
{
	CPropTreeItemStatic* pItem = new CPropTreeItemStatic;
	return pItem;
}